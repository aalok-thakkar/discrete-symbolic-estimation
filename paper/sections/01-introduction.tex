...
\item \emph{Implementation and evaluation.}  We implement \toolname{}
  as a Python prototype with a Z3 backend and evaluate it on
  twelve integer benchmarks drawn from established sources, plus a
  parametric rare-event experiment.  Two ablations isolate the
  contribution of the bound and of the symbolic stratification.  On
  the rare-event sweet spot $\mutrue \in [5{\times}10^{-4}, 2{\times}10^{-3}]$
  \toolname{} certifies $\mutrue$ to half-width exactly zero in
  $\Theta(1)$ samples---a regime where plain Monte Carlo's
